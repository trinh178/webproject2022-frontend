...
import {
  EduCourseProps,
  EduCourseReportProps,
  EduReportType,
} from "services/course/types";
import CourseIntro from "../components/CourseIntro";
import SamuiSlideProvider, {
  SamuiSlideControlsProps,
} from "shared/components/SamuiSlideProvider";
...
export function getCourseReportByLatest(): EduCourseReportProps {
  const latestCourseReport = JSON.parse(
    localStorage.getItem("__latestCourseReport")
  ) as EduCourseReportProps;
...
export function createCourseReport(
  ...
  if (type === "STUDY_FROM_SCRATCH") {
    ...
    cr = {
      ...
      contentReports: latestCourseReport.contentReports.map((c) => ({
        questionsResults: c.questionsResults.map((q) =>
          c.studyStatus === "STUDIED_UNFINISHED" ? false : q
        ),
        studyStatus:
          c.studyStatus === "STUDIED_UNFINISHED"
            ? "NOT_STARTED"
            : c.studyStatus,
      })),
      studyStatus: "STUDYING", // Start study status for course
  ...
  localStorage.setItem(
    "__totalCourseReports",
    (
      (Number.parseInt(localStorage.getItem("__totalCourseReports")) || 0) + 1
    ).toString()
  );
  return cr;
}
...
export default function Loader() {
  ...
  const [initialStudyAction, setInitialStudyAction] =
    React.useState<StudyAction>(null);
